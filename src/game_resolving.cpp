...
{ 
    ...
    ros::Publisher coord_pub = nh.advertise<camelot_jr::MouveCoords>("/PieceCoords", 1000);
    ...
    // **** Acquisition des donnees ****
        ...
        // ***** Recherche de solution *******
                ...
                if ( ! sortedPieces.empty() )
                {
                    ...
                    // Pose de la piece a deplacer
                    camelot_jr::MouveCoords controlerCoord;
                    // pose de depart
                    int idDepart = sortedPieces[0].getId();
                    bool trouve = false;
                    int var = 0;
                    while(!trouve && (var <marray.markers.size()))
                    {
                        if ( marray.markers[var].id == idDepart ) 
                        {
                            trouve = true;
                            controlerCoord.depart.position.x = marray.markers[var].pose.pose.position.x;
                            controlerCoord.depart.position.y = marray.markers[var].pose.pose.position.y;
                            controlerCoord.depart.position.z = marray.markers[var].pose.pose.position.z;
                        }
                        var++;

                    }
                    // pose de destination
                    controlerCoord = toPose(sortedPieces[0].getPosX(),sortedPieces[0].getPosY());

                    //publication sur le topic PieceCoords
                    coord_pub.publish(controlerCoord);
